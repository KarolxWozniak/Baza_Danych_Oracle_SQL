...
create or replace view TOP_FAKTURY1 as
  select top 3 * from  TOP_FAKTURY;

create or replace view WYKAZ_FAKTUR as
  select distinct fa.ID_FAKTURY as "Numer faktury", DATA_FAKTURY as "Data faktury", cast(round(WARTOC_FAKTURY/1.23 , 2) as numeric (10,2)) as "Wartosc NETTO",
WARTOC_FAKTURY as "Wartosc BRUTTO", IMIE ||' '|| NAZWISKO as "Klient" 
...
create or replace view WYKAZ_CZESCI as
  select  NAZWA_CZESCI as "Nazwa Czesci", MARKA_SAMOCHODU ||' '|| MODEL_SAMOCHODU as "Samochod", cast(round(CENA_CZESCI/1.23 , 2) as numeric (10,2)) as "Cena NETTO",
 CENA_CZESCI as "Cena BRUTTO" 
